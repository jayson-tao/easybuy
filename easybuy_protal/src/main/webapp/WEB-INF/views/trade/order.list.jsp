<%@page import="com.yaorange.consts.bis.OrderStateConsts"%>
<%@page import="com.yaorange.consts.bis.PayChannelConsts"%>
<%@page import="com.yaorange.consts.ConstUtils"%>
	...
	<tr class="trade-order-info">
		<td colspan="2">
			<label>
				...
				<span style="">${orderStatus['date'.concat(order.id)]}</span>
			</label>
			<span>订单号：</span>
		...
		<td colspan="4">
			...
			<a href="javascript:;" title="易购物官方自营二号店" class="shop-name">易购物官方自营二号店</a>
...
<tr>
	<td class="goods-info">
		<div style="overflow: hidden;">
			<a class="goods-img" href="http://127.0.0.1/p/${sku.productId}?skuId=${sku.skuId}" target="_blank">
				<img src="http://pijbgt8k2.bkt.clouddn.com/${sku.skuMainPic}?imageView2/1/w/80/h/80">
			</a>
			<div class="item-con">
				<div class="item-name">
					<a href="http://127.0.0.1/p/${sku.productId}?skuId=${sku.skuId}" target="_blank">
						<span>${sku.name}</span>
					</a>
				</div>
	...
	<td class="trading-status" rowspan="${order.detailList.size()}">
		<div class="operate"> <a>
		${orderStatus["stateName".concat(order.id)]}
		</a> </div>
		<div class="operate">
	...
	<td class="trading-operate" rowspan="${order.detailList.size()}">
				...
				<script type="text/javascript">
					$(document).ready(function() {
						$("#counter_${order.id}").countdown({
							date: "${orderStatus['payDate'.concat(order.id)]}",
		...
		<p class="confirm-receipt-time">
				...
				<script type="text/javascript">
					$(document).ready(function() {
						$("#counter_${order.id}").countdown({
							date: "${orderStatus['lastConfirmDate'.concat(order.id)]}",
							onChange: function(event, timer) {},
							onComplete: function(event) {
								$(this).html("已超时！");
								//location.href = '/trade/order';
...
<script>
var staOrderJson = $.parseJSON('${allKindsOfNum}');
